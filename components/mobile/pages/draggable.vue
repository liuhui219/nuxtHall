...
<template>
    <client-only
        ><div
            ...
            v-if="show"
        >
            <div class="cursor-pointer" @click="callBackFn">
                <div v-if="spines" :id="container" class="h-[60px] w-[60px]"></div>
                <div v-if="!spines">
            ...
            <div v-if="countdown" class="redpacket-text flex justify-center">
                <el-countdown title="" :value="value" />
            </div></div
    ></client-only>
</template>
...
<script setup>
    ...
    onMounted(() => {
        if (!props.spines) return;
        setTimeout(() => {
            let jsonUrl = `./lottie/${props.spineName}.json`;
            let atlasUrl = `./lottie/${props.spineName}.atlas`;
            new spine.SpinePlayer(container.value, {
                jsonUrl, //json文件
                atlasUrl,
